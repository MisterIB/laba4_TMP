...
const OnboardControl = () => {
    ...
    useEffect(() => {
        const fetchOCS = async () => {
            try {
                const response = await axios.get(`http://217.71.129.139:5735/onboard_control/${id}`, { withCredentials: true, credentials: 'include'})
    ...
    const handleSave = async () => {
        try {
            await axios.put(`http://217.71.129.139:5735/onboard_control/${id}`, ocs, { withCredentials: true, credentials: 'include' })
            setIsEditing(false)
        } catch (err) {
            console.log(err)
